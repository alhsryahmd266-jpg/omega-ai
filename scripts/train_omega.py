...
def main():
    ...
    if is_ci:
        ...
        print("AION LARGE mode (16GB target)")

    data_path = 'data/training_data.json'
    samples = json.load(open(data_path,'r',encoding='utf-8')) if os.path.exists(data_path) else get_training_data()

    trainer = OmegaTrainer(cfg, tc)
    trainer.run(samples, epochs=tc['epochs'])

    os.makedirs('checkpoints', exist_ok=True)
    cfg_dict = {k: v for k, v in cfg.__dict__.items() if not k.startswith('_') and not callable(v)}
    with open('checkpoints/config.json','w') as f:
        json.dump(cfg_dict, f, indent=2)

    # Rename checkpoint to aion_best
    import shutil
